...
function sendWhatsApp(){
  ...
  let msg = `🧾 Order #${orderId}%0A`;
  msg += `👤 Name: ${name}%0A`;
  msg += `📞 Phone: ${phoneInput}%0A`;
  ...
  cart.forEach(i => {
    msg += `• ${i.name} x${i.qty} = ${i.price * i.qty} EGP%0A`;
    total += i.price * i.qty;
  });

  msg += `----------------%0A💰 Total: ${total} EGP`;

  let phone = "201102579732"; // رقمك بصيغة دولية

  window.open(`https://wa.me/${20102579732}?text=${msg}`);
}
